(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(require('fs'), require('webpack'), require('source-map'), require('@vue/cli-service/webpack.config'), require('@babel/parser'), require('@babel/traverse'), require('@babel/types')) :
  typeof define === 'function' && define.amd ? define(['fs', 'webpack', 'source-map', '@vue/cli-service/webpack.config', '@babel/parser', '@babel/traverse', '@babel/types'], factory) :
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.codeScanner = factory(global.fs, global.webpack, global.sourceMap, global.baseWebpackConfig, global.babelParser, global.babelTraverse, global.babelTypes));
}(this, (function (fs, webpack, require$$0, baseWebpackConfig, parser, traverse, types$1) { 'use strict';
  ...
  **/
  ...
  var wildcard = function wildcard(text, test, separator) {
  ...
  var various_customPlg = function various_customPlg(babel) {
    ...
    return {
      ...
      visitor: {
        MemberExpression: function MemberExpression(path, state) {
          ...
          if (num > 1) {
            var line = path.node.loc ? path.node.loc.start.line : '';
            var column = path.node.loc ? path.node.loc.start.column : '';

            if (!line || !column) {
              return;
            }

            var location = "---trace: line ".concat(line, ", column ").concat(column, ", ").concat(state.filename, "---");
            console.log(location);
            console.log('********************写入日志文件*******************');
            logs = logs + '\n' + JSON.stringify({
              line: line,
    ...
    module: {
      rules: [{
        ...
        use: {
          loader: 'babel-loader',
          options: {
            plugins: [customPlugin, various_customPlg],
  ...
  var CodeScanner = /*#__PURE__*/function () {
    ...
    _createClass(CodeScanner, [{
      ...
      value: function run(basePath) {
        ...
        var babelLoaderCachePath = "".concat(basePath, "/node_modules/.cache/babel-loader");

        if (fs__default['default'].existsSync(babelLoaderCachePath)) {
